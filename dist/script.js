  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      ...
      document.querySelectorAll('[data-modal]').forEach(item => {
        // проверяем computed style, а не inline
    ...
    link.addEventListener('click', function (event) {
      ...
      function step(time) {
        ...
        let progress = time - start,
            // вычислить на сколько пикселей необходимо продвинуть
        // анимацию и в какую сторону
...
const showMoreStyles = (trigger, wrapper) => {
  const btn = document.querySelector(trigger); // через работу с версткой и стилями
  // cards.forEach(card => {
